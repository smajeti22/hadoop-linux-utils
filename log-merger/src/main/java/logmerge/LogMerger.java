	...
	public void merge() {
		...
		try {
            ...
            while (nextLogFile != null) {
                String markerString = "[" + nextLogFile.getMarkerFileName() != null ? nextLogFile.getMarkerFileName() : nextLogFile.getMarker() + "]" + cliOptions.getDelimiter();
    ...
    private List<LogFile> createLogFiles(CliOptions cliOptions) {
		...
		for (String fileName : cliOptions.getLogFiles()) {
			try {
                ...
                LogFile logFile = new LogFile(fileName, fileReader, simpleDateFormat, cliOptions, fileName);
